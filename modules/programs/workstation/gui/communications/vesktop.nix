...
in {
  flake.nixosModules.vesktop = { pkgs, lib, ...}: {
    ...
    preservation.preserveAt."/persistent" = {
      users.shot = {
        ...
        files = [
          ...
          ".config/vesktop/sessionData/Cookies-journal"
    ...
    home-manager.users.shot = { lib, ... }: {
      ...
      home.activation.setupVesktop = lib.hm.dag.entryAfter ["writeBoundary"] ''
        mkdir -p $HOME/.config/vesktop/settings

        cp -f ${pkgs.writeText "vesktop-app-settings.json" ''
          {
            "discordBranch": "stable",
            "minimizeToTray": false,
            "arRPC": false,
            "splashColor": "#${c.greym}",
            "splashBackground": "#${c.black}"
          }
        ''} $HOME/.config/vesktop/settings.json
        chmod 644 $HOME/.config/vesktop/settings.json

        # Copy settings.json and make it writable
        # settings/settings.json {{{
        cp -f ${pkgs.writeText "vesktop-settings.json" ''
{
    "autoUpdate": true,
    ...
    "cloud": {
        ...
        "settingsSync": false,
        "settingsSyncVersion": 1786045046742
    }
}
      ''} $HOME/.config/vesktop/settings/settings.json
# }}}
        chmod 644 $HOME/.config/vesktop/settings/settings.json

        # Copy quickCss.css and make it writable
        # quickCss {{{
        cp -f ${pkgs.writeText "vesktop-quickCss.css" ''
/* https://github.com/imbypass/base16-discord/blob/c98a7a19371e085a441d257ced50e52e923d4160/base16.css */
:root {
    --font: "Hack Nerd Font Mono";
    --font-code: "Hack Nerd Font Mono";
...
.visual-refresh {
    ...
    --mention-background: var(--base02) !important;
    --mention-foreground: var(--base04) !important;
    ...
    --text-brand: var(--base06) !important;
...
# }}}
        chmod 644 $HOME/.config/vesktop/settings/quickCss.css
      '';
    };
  };
}
